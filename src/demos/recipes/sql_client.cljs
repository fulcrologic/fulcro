...
(defn random-person []
  (let [age  (inc (rand-int 30))
    ...
    {:id (prim/tempid) :name name :age age}))
...
(defui ^:once Root
  ...
  Object
  (render [this]
    (let [{:keys [ui/react-key people]} (prim/props this)]
      (dom/div #js {:key react-key}
        (dom/p nil "The people:")
        (dom/button #js {:onClick (fn [] (prim/transact! this `[(app/add-person ~(random-person))]))} "Add Random Person")
        (dom/ul nil
