  ...
  * `ns` - the ns in which to load the translations
  ...
  (let [trans-ns    (symbol (str ns "." locale))
        ...
        ns-decl     (pp/write (list 'ns trans-ns
                                (list :require 'fulcro.i18n 'cljs.loader)) :pretty false :stream nil)
        ...
        loaded-decl (pp/write (list 'cljs.loader/set-loaded! locale) :stream nil)]
    (str/join "\n\n" [ns-decl comment trans-def swap-decl loaded-decl])))
...
(defn deploy-translations
  ...
  (let [{:keys [existing-po-files output-path outdir]
         :as   settings} (some-> {:src src :ns ns :po po}
                           expand-settings
                           verify-po-folders
    ...
    (doseq [po existing-po-files]
      (let [locale            (clojure-ize-locale po)
            ...
            cljc-translations (wrap-translations-in-ns :ns ns :locale locale :translation translation-map)
