(ns com.fulcrologic.fulcro.pure.hiccup
  "Hiccup conversion and utilities for pure DOM elements.

   This namespace provides:
   - Conversion from pure/dom Element records to hiccup format
   - Utility functions for inspecting and traversing hiccup trees
   - Functions for simulating user interactions (click, type, etc.)
   ...
   (require '[com.fulcrologic.fulcro.pure.hiccup :as hic])

   ;; Create a pure DOM element
   (def elem (dom/div {:id \"my-div\" :onClick #(println \"clicked\")}
               (dom/span {} \"Hello\")))

   ;; Convert to hiccup
   (def h (hic/element->hiccup elem))
   ;; => [:div {:id \"my-div\" :onClick #fn} [:span {} \"Hello\"]]

   ;; Inspect and interact
   (hic/find-by-id h \"my-div\")
   (hic/click! (hic/find-by-id h \"some-btn\"))
   ```"
  (:require
...
;; =============================================================================
;; Hiccup Conversion from pure/dom Elements
;; =============================================================================
...
;; In CLJC we can't easily extend protocols to records across platforms,
;; so we use a multimethod-style dispatch based on type checks instead.

(declare to-hiccup*)

(defn- element-to-hiccup [{:keys [tag attrs children]}]
...
(defn- text-to-hiccup [{:keys [s]}] s)

(defn- react-text-to-hiccup [{:keys [text]}] text)

(defn- react-empty-to-hiccup [_] nil)

(defn- react-fragment-to-hiccup [{:keys [elements]}]
  (let [converted (keep to-hiccup* elements)]
    (vec converted)))

(defn to-hiccup*
  "Convert a pure/dom element to hiccup. Dispatches based on record type."
  [x]
  (cond
    (nil? x) nil
    (dom/element? x) (element-to-hiccup x)
    (dom/text? x) (text-to-hiccup x)
    (dom/react-text? x) (react-text-to-hiccup x)
    (dom/react-empty? x) (react-empty-to-hiccup x)
    (dom/react-fragment? x) (react-fragment-to-hiccup x)
    :else (throw (ex-info "Unknown element type for hiccup conversion" {:type (type x) :value x}))))

(defn element->hiccup
  "Convert a pure/dom Element tree to hiccup format.
   Preserves all attributes including function handlers.

   Accepts:
   - A pure/dom Element record
   ...
   Example:
   ```clojure
   (element->hiccup (dom/div {:id \"test\" :onClick my-handler} \"Hello\"))
  ...
  (cond
    ...
    ;; Vector of elements - recursively convert
    (vector? x)
    (mapv element->hiccup x)

    ;; Try to convert as a pure/dom element type
    :else
    (to-hiccup* x)))

(def rendered-tree->hiccup
  "Alias for `element->hiccup` for consistency with headless/hiccup.clj"
  element->hiccup)
...
(defn click!
  "Invoke the :onClick handler of an element.
   ...
   Handlers are automatically wrapped to be arity-tolerant during hiccup conversion.
...
(defn invoke-handler!
  "Invoke a specific handler on an element.
   ...
   Any additional arguments are applied to the handler.
...
(defn element-exists?
  "Returns true if an element with the given ID exists in the tree.
   ...
   Example:
   ```clojure
   (element-exists? tree \"submit-btn\") => true
   ```"
  [tree id]
...
(defn- text-pattern-matches?
  ...
  (when text
    (cond
      ...
      #?(:clj  (instance? java.util.regex.Pattern pattern)
         :cljs (regexp? pattern)) (boolean (re-find pattern (str text)))
      :else false)))
