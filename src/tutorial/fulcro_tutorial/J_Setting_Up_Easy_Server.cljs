...
(defcard-doc
  ...
  Your component should pull the value of `:n` from this file and log it (using `log/info`).
  ...
  Then, in `src/server/app/api.clj` add a mutation (use `defmethod`) that logs your configuration value `:sample :n` using log from the
