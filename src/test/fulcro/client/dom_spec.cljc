...
#?(:clj
   (specification "Macro processing"
     (assertions
       ...
       (jsvalue->map (#'fulcro.client.dom/emit-tag "div" [:.a {:data-x 1} "Hello"]))
       => `(fulcro.client.dom/macro-create-element*
             {:jsvalue ["div"
                        {:jsvalue {:data-x    1
                                   :className "a"}}
       ...
       (jsvalue->map (#'fulcro.client.dom/emit-tag "div" [:.a '(props-map) "Hello"]))
       => `(fulcro.client.dom/macro-create-element "div" [~'(props-map) "Hello"] :.a)
     ...
     (specification "Wrapping form elements"
       (let [element          (atom nil)
         ...
         (behavior "a simple input tag"
           (reset! element (.mount enzyme simple-input-tag))
           (-> @element
             (.find "input")
         ...
         (behavior "an input tag nested inside other tags"
           (reset! element (.mount enzyme nested-input-tag))
           (-> @element
             (.find "input")
             (.simulate "change" (clj->js {:target {:value new-value}})))
