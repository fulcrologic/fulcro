(ns fulcro.server.impl.components.config-spec
  (:require [com.stuartsierra.component :as component]
            [fulcro.server :as cfg :refer [new-config raw-config]]
...
(t/use-fixtures
  :once #(timbre/with-merged-config
           {:ns-blacklist ["fulcro.server.impl.components.config"]}
           (%)))
  ...
  (component "resolve-symbol"
    (behavior "requires if necessary"
      (when-mocking
        ...
        (assertions (#'cfg/resolve-symbol 'fulcro.server.fixtures.dont-require-me/stahp) => false)))
    ...
    (behavior "if not found in the namespace after requiring"
      (assertions
        (#'cfg/resolve-symbol 'fulcro.server.fixtures.dont-require-me/invalid) =throws=> (AssertionError #"not \(nil")))
...
(specification "fulcro.server.impl.components.config"
