...
(defn trim-text
  ...
  (let [trimmed-text (str/trim text)]
    (when (seq trimmed-text)
      trimmed-text)))

(defn- event-target-value
  "Extract the target value from an event. In CLJS this is a DOM event, in CLJ a map."
  [evt]
  #?(:cljs (.. evt -target -value)
     :clj  (get-in evt [:target :value])))
...
(defsc TodoItem [this
  ...
  {:query [:item/id :item/label :item/complete :ui/editing :ui/edit-text]
   :ident :item/id
   #?@(:cljs [:initLocalState (fn [this] {:save-ref (fn [r] (gobj/set this "input-ref" r))})
  ...
  (let [submit-edit (fn [evt]
                      (if-let [trimmed-text (trim-text (event-target-value evt))]
                        (do
    ...
    (dom/li {:classes [(when complete (str "completed")) (when editing (str " editing"))]}
      ...
      (dom/input {:ref       #?(:cljs (comp/get-state this :save-ref)
                                :clj  nil)
                  :className "edit"
                  :value     (str edit-text)
  ...
  (let [{:keys [list/id ui/new-item-text]} (comp/props component)]
    (dom/header :.header {}
      ...
      (dom/input {:value       (str new-item-text)
                  ...
                  :onKeyDown   (fn [evt]
                                 (when (is-enter? evt)
                                   (when-let [trimmed-text (trim-text (event-target-value evt))]
...
#?(:cljs
   (comment
     (require '[fulcro-todomvc.app :refer [app]])
