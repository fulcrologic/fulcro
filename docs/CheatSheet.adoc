...
In 1.x, the primary mechanism for defining components was defui. In 2.x+ it is still fully-supported, but see
defsc for a more concise syntax.
...
==== Components with `defsc`

The `defsc` macro builds a `defui` with a query, ident (optional), initial state (optional)
...
(defsc Person
  "Docstring"
  ; IMPORTANT: The css-classnames-map should only be used if using :css or :css-include. Otherwise drop that parameter.
  [this {:keys [person/id person/name] :as props} {:keys [onSelect] :as computed-props} css-classnames-map children]
  { :query (fn [] [:person/id :person/name {:person/job (prim/get-query Job)}])
    :ident (fn [] [:PERSON/by-id id] ; id comes from destructured props
    :css (fn [rules...]) ; garden css
    :css-include (fn [children...]) ;see fulcro-css
...
(defsc Person
  "Docstring"
  ; IMPORTANT: The css-classnames-map should only be used if using :css or :css-include. Otherwise drop that parameter.
  [this {:keys [person/name] :as props} {:keys [onSelect] :as computed-props} css-classnames-map children]
