...
(defmutation next-page [params]
  (action [{:keys [state] :as env}]
    ...
    (df/load-action state :prop Component {:remote :remote}) ; same basic args as `load`, except state atom instead of `this`
    (df/load-action state :other Other) {:remote :other-remote}) ; as many as you need...
  (remote [env]
