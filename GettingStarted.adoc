...
; A pretend server-side database of people
(def people-db (atom {1 {:db/id 1 :person/name "Server Joe" :person/age 55 :person/relation :friend}
                      ...
                      99 {:db/id 99 :person/name "Me" :person/age 35}))
----
...
Of course, the question is now "how do I use that in some arbitrary component?" We won't explore that right now,
but the answer is easy: The query syntax has a notation for "query something at the root". It looks like this:
...
#### Loading something from a legacy REST API

* TODO: Add some kind of pluggable networking object that you can extend
to handle rest API calls???
