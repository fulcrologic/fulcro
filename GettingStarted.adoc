...
----
  :profiles {:dev {:source-paths ["src/dev" "src/main"]
                   ...
                   :dependencies [[binaryage/devtools "0.9.4"]
                                  ...
                                  [org.clojure/tools.namespace "0.3.0-alpha4"]
                                  [figwheel-sidecar "0.5.11" :exclusions [org.clojure/tools.nrepl]]
                                  [org.clojure/tools.nrepl "0.2.13"]]}})
----
* With `src/def/user.clj` open in a buffer, choose `M-x cider-jack-in-clojurescript` or `C-c M-J` (`M-J` with capital J).

Two repls will open up, one for clojure, one for clojurescript.
...
Fulcro uses Om's `defui` to build React components. This macro emits React components that work as 100% raw React
  ...
  (initial-state [comp-class {:keys [label]}]
    {:person-list/label  label
     :person-list/people (if (= label "Friends")
                           [(fc/get-initial-state Person {:name "Sally" :age 32})
                            (fc/get-initial-state Person {:name "Joe" :age 22})]
                           [(fc/get-initial-state Person {:name "Fred" :age 11})
                            (fc/get-initial-state Person {:name "Bobby" :age 55})])})
  Object
  (render [this]
    (let [{:keys [person-list/label person-list/people]} (om/props this)]
      (dom/div nil
...
(defui ^:once Root
  static
  fc/InitialAppState
  (initial-state [c params] {:friends (fc/get-initial-state PersonList {:label "Friends"})
                             :enemies (fc/get-initial-state PersonList {:label "Enemies"})})
  Object
  (render [this]
    (let [{:keys [ui/react-key]} (om/props this)
          {:keys [friends enemies]} (fc/get-initial-state Root {})]
      (dom/div #js {:key react-key}
...
[source]
----
dev:cljs.user=> (fulcro.client.core/get-initial-state app.basic-ui/Root {})
{:friends
...
NOTE: Technically, in cljs you can call `fulcro.client.core/initial-state` directly, but this doesn't work right when
...
[source]
----
dev:cljs.user=>  (om.next/db->tree [{:friends [:person-list/label]}] (fulcro.client.core/get-initial-state app.basic-ui/Root {}) {})
...
detail in the Om documentation and the Fulcro Developer's Guide, but Fulcro provides a macro that makes
  ...
  (initial-state [comp-class {:keys [id label]}]
    {:db/id              id ; <5>
     ...
     :person-list/people (if (= id :friends)
                           [(fc/get-initial-state Person {:id 1 :name "Sally" :age 32}) ; <3>
                            (fc/get-initial-state Person {:id 2 :name "Joe" :age 22})]
                           [(fc/get-initial-state Person {:id 3 :name "Fred" :age 11})
                            (fc/get-initial-state Person {:id 4 :name "Bobby" :age 55})])})
  Object
  (render [this]
...
(defui ^:once Root
  ...
  static
  fc/InitialAppState
  (initial-state [c params] {:friends (fc/get-initial-state PersonList {:id :friends :label "Friends"}) ; <5>
                             :enemies (fc/get-initial-state PersonList {:id :enemies :label "Enemies"})})
  Object
  (render [this]
    ; NOTE: the data now comes in through props!!!
    (let [{:keys [ui/react-key friends enemies]} (om/props this)]
...
about that in the developer's guide; however, remember that we're using Fulcro's built-in request parser. This
...
(defui ^:once Root
  ...
  static
  fc/InitialAppState
  (initial-state [c params] {:friends (fc/get-initial-state PersonList {:id :friends :label "Friends"})
                             :enemies (fc/get-initial-state PersonList {:id :enemies :label "Enemies"})})
  Object
  (render [this]
    ; NOTE: the data now comes in through props!!!
...
(defui ^:once Root
  ...
  static
  fc/InitialAppState
  (initial-state [c params] {:blog-posts (fc/get-initial-state Posts {}) ; <4>
                             :friends    (fc/get-initial-state PersonList {:id :friends :label "Friends"})
                             :enemies    (fc/get-initial-state PersonList {:id :enemies :label "Enemies"})})
  Object
  (render [this]
    ; NOTE: the data now comes in through props!!!
