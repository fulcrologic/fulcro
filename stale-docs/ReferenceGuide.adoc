...
The `defsc` macro generate a component in Fulcro. It allows for both a template-based set of options, and
lambda-based options. The overall syntax is:

```
(defsc ComponentName [this props computed css-classes]
  { ...options... }
  DOM)
```
...
* `:ident` - A two-element vector. The first element is literal (table name) and the second is a keyword of the ID
prop for the component. Can also be a lambda that closes over the `this` and `props` from the component's argument
list.
* `:query` - A vector (Fulcro query) that can contain joins (via `get-query`) on children. May also be a lambda if
additional code is needed within the body.
* `:initial-state` - A map of initialization data or a lambda of `(fn [params])`. If a map is used, then children joined
in the query should be initialized with their parameters (*not* a call to `get-initial-state`). With a lambda,
any code is allowed.
* `:css` - If you're using Fulcro-CSS, a vector of Garden CSS rules
* `:css-children` - If using Fulcro-CSS, a vector of components whose CSS should be included. This allows compositional
logic for your CSS.
...
** `:protocols [Protocol methods Protocol methods ...]` Causes resulting class to
implement the given protocols. Use `Object` protocol to add methods to the resulting
component class.
...
`(fulcro.client.util/react-instance? Component instance)`:: Returns true if the given element is an instance of the given component class. Otherwise `nil`.
...
(defsc D3Thing [this props]
  ...
  :componentDidMount (fn [] (d3-render (dom/node this) (prim/props this)))
  ...
  :componentWillReceiveProps (fn [props] (d3-render (dom/node this) props))
  }
...
;; TODO: CONTINUE HERE
(defsc Child [this {:keys [size]}]
  {
  :initial-state {:id 0 :size 50 :marker [0.5 0.5]}
  :query  [:id :size :marker]
  :ident [:child/by-id :id]
  :initLocalState (fn [] {:coords [-50 -50]})
  }
  ; Remember that this "rendering" just renders the DOM (e.g. the canvas DOM element). The graphical rendering within the canvas is done during event handling.
